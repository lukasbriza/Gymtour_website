...
const Modify: FC = () => {
  ...
  const { fetchCall: fetchCoach, loading: coachLoading } =
    useServerdataLazy(getCoaches);
  const { fetchCall: fetchFitness, loading: fitnessLoading } =
    useServerdataLazy(getFitnesses);
  const { fetchCall: fetchImage, loading: imageLoading } =
    useServerdataLazy(getImage);
  const [providerObject, setProviderObject] =
    useState<TransformedProviderObject<Coach | Fitness>>();
    ...
    if (id && type) {
      ...
      response.then(async (value) => {
        if (value?.data) {
          ...
          const main: Blob = (await fetchImage({
            id: pics.detail.main,
          })) as Blob;
          let others: File[] = [];
